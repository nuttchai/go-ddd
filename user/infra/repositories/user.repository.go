...
func (r *UserRepository) UpdateFirstNameIfIdExist(id, firstName string) error {
	...
	mapper := r.Repository.DataMapper()
	userDAL, err := mapper.ToDalEntity(user)
	if err != nil {
		return err
	}
...
func (r *UserRepository) FindOneByEmail(email string) (*entity.User, error) {
	...
	if dbResult := r.Repository.QueryAdapter().Find(&userDAL).First(&userDAL); dbResult.Error != nil {
	...
	mapper := r.Repository.DataMapper()
	return mapper.ToDomainEntity(&userDAL)
}
